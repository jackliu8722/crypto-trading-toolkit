...
struct OrderIntent {
    ...
    legs:        Vec<Leg>,      // 1~2 腿
    objective:   Objective,     // 完成率 / 价格改善 / 低冲击 / 综合成本
...
`core` 不依赖任何上层。**发现需要反向依赖 = 抽象位置错了，回来改结构。**
...
- 价格、数量、金额、费用、保证金使用精确十进制，**禁止 `f64`**。f64 只用于统计指标与图形。
- **精确运算失败必须返回错误，禁止静默近似或舍入**（如 `1/3` 应显式失败）。静默近似是资金差错的头号来源。
- 溢出、下溢、除零、负值行为必须显式定义。
...
两腿是**补偿事务，不是原子事务**：一腿成交另一腿失败时进入预授权的补偿路径，按既定策略处理已成交腿。决策输入含盘口深度、手续费模型、滑点估计、资金费率、可用额度。
...
- 首期通过 `GetTask` 快照查询状态；历史事件回放按真实需求后置。
